...
def insert(rrList):
	...
	for rr in rrList:
		if rr[2] == 'NS':
			...
			#print('A record:\n\thost = %s\n\tttl = %s\n\tdata = %s\n' % (rr[0], rr[1], rr[3]))
			cur.execute('''insert into a (zone_, ttl, host, address) values (%s, %s, %s, %s);''', (zone, rr[1], rr[0], rr[3]))
			sys.stdout.write('a')
		elif rr[2] == 'AAAA':
			#print('AAAA record:\n\thost = %s\n\tttl = %s\n\tdata = %s\n' % (r[0], r[1], r[3]))
			cur.execute('''insert into aaaa (zone_, ttl, host, address) values (%s, %s, %s, %s);''', (zone, r[1], r[0], r[3]))
			sys.stdout.write('A')
		else:
			sys.stdout.write('-')
